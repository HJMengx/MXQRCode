...
//        右下角水平线
        ...
        // add animation
        if self.viewStyle.anmiationStyle == .LineMove {
            self.scanLineAnimation?.frame = CGRect.init(x: scanRetangleRect.origin.x, y: scanRetangleRect.origin.y, width: scanRetangleRect.width + 2 * diffAngle, height: 25)
        }
    }
